    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Dialog open={isBlogDialogOpen} onOpenChange={(open) => {
          ...
          if (!open) { setEditingBlog(null); setImagePosition("top"); }
        }}>
          <DialogTrigger asChild>
            <Button onClick={() => { setEditingBlog(null); setImagePosition("top"); }} className="gap-2">
          ...
          <DialogContent className="bg-card max-h-[95vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveBlog} className="space-y-4">
              ...
              <div className="space-y-2 border border-border rounded-xl p-4 bg-muted/30">
                ...
                />
                <Label className="mt-3 block">Image Position</Label>
                <Select
                  value={imagePosition}
                  onValueChange={(v) => setImagePosition(v as "top" | "middle" | "bottom")}
                >
                  <SelectTrigger>
      ...
      <CardContent>
        {error ? (
          ...
          <p className="text-muted-foreground text-center py-6">No Blog yet. Click "Add Post" to create one.</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {blogPosts.map((post) => (
                <TableRow key={post.id}>
                  ...
                  <TableCell>
                    {post.imageUrl
                      ? <span className="text-xs text-primary font-medium">{post.imagePosition}</span>
